					...
					<div class="card shadow mb-4">
						<div class="card-header py-3">
							<a style="text-decoration: none;"
								class="m-0 font-weight-bold text-primary">Board readView</a>
						</div>
						<br />
						<div class="container">
							...
							<section id="container">
								...
								<div class="form-group">
									<label style="font-weight: bold;" for="title"
										class="col-sm-2 control-label">제목</label> <input type="text"
										id="title" name="title" class="form-control"
										value="${read.title}" readonly="readonly" />
								</div>

								<div class="form-group">
									<label style="font-weight: bold;" for="content"
										class="col-sm-2 control-label">내용</label>
									<div
										style="border: 1px solid; padding: .435rem .75rem; border-radius: .35rem; background-color: #eaecf4; border-color: #d1d3e2">
										${read.content}</div>
								</div>

								<div class="form-group">
									<label style="font-weight: bold;" for="writer"
										class="col-sm-2 control-label">작성자</label> <input type="text"
										id="writer" name="writer" class="form-control"
										value="${read.userName}" readonly="readonly" />

								</div>

								<div class="form-group">
									<label style="font-weight: bold;" for="regdate"
										class="col-sm-2 control-label">작성날짜</label>
									<fmt:formatDate value="${read.regdate}"
										pattern="yyyy-MM-dd HH:mm:ss" />
								</div>

								<div class="form-group">
									<label style="font-weight: bold;" for="editor"
										class="col-sm-2 control-label">수정자</label> <input type="text"
										id="editor" name="editor" class="form-control"
										value="${read.userName}" readonly="readonly" />

								</div>
								<div class="form-group">
									<label style="font-weight: bold;" for="editdate"
										class="col-sm-2 control-label">수정날짜</label>
									<fmt:formatDate value="${read.editdate}"
										pattern="yyyy-MM-dd HH:mm:ss" />
								</div>
								...
								<div class="form-group" style="border: 0px solid #dbdbdb;">
									<c:forEach var="file" items="${file}">
										<a href="#"
											onclick="fn_fileDown('${file.FILE_NO}'); return false;">${file.OGN_FILE_NAME}</a> ( 파일크기 : ${file.FILE_SIZE}KB )<br>
									</c:forEach>
								</div>
								<hr>
								...
								<div style="float: right;">
									<c:if test="${read.writer == member.userId}">
										<button type="submit"
											class="update_btn btn btn-outline-primary">게시물 /
											첨부파일 수정</button>
									</c:if>
									...
									<div class="replyList">
										...
										<c:forEach items="${replyList}" var="replyList">
											<c:choose>
												<c:when test="${replyList.deleted == 'N'}">
													<div>
														<label style="font-weight: bold;"> <!-- 작성자 :  -->${replyList.userName}
														</label> <label
															style="float: right; font-weight: 300; font-size: 11px;">
															<!-- 작성날짜 :  -->
						   			...
						   			수정날짜 : <fmt:formatDate value="${replyList.editdate}" pattern ="yyyy-MM-dd HH:mm:ss" />
													...
													<c:if test="${replyList.userName == member.userName}">
														...
														<div style="float: right;">
															<button style="color: #4e73df; font-weight: 600;"
																type="button"
																class="replyUpdateBtn btn btn-outline-error"
																data-rno="${replyList.rno}">수정</button>
															&nbsp;/
															<button style="color: #4e73df; font-weight: 600;"
																type="button"
																class="replyDeleteBtn btn btn-outline-error"
													...
													<c:if test="${replyList.userName != member.userName}">
														...
														<p
															style="font-weight: 100; font-size: 5px; color: red; float: right;">*
															작성자만 수정/삭제 가능 *</p>
														<br>
														<hr />
													</c:if>
								...
								<!-- 댓글 작성 -->
								<c:if test="${member.userId != null}">
									<p style="font-weight: 900; color: #4e73df; font-size: 20px;">댓글
										작성</p>
									...
									<form name="replyForm" id="replyForm" method="post">
										...
										<div>
											<label style="font-weight: bold;" for="userno"
												class="col-sm-2 control-label">댓글 작성자</label> <label
												style="border: 1px solid; padding: .435rem .75rem; border-radius: .35rem; background-color: #eaecf4; border-color: #d1d3e2"
												for="userno" class="form-control">${member.userName}</label>
											<input type="hidden" id="userno" name="userno"
												value="${member.userNo}" /> <br /> <label
												style="font-weight: bold;" for="content"
												class="col-sm-2 control-label">댓글 내용</label>
											<textarea placeholder="댓글 내용을 입력해주세요..." id="content"
												name="content" maxlength="500" class="form-control content"
										...
										<div>
											<button type="button" onclick="validateForm()"
												class="btn btn-outline-primary">작성</button>
										</div>
									</form>
								</c:if>
								<c:if test="${member.userId == null}">
